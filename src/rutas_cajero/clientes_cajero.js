...
router.get("/clientes", requireAuth, async function (req, res) {
  pool.query(
    "SELECT * FROM clientes",
    function (error, results, fields) {
      if (error) throw error;
      res.render("clientes_cajero", { clientes: results });
    }
  );
});
  ...
  try {
    ...
    pool.query(sqlBuscaCliente, [dni, ruc, telefono], async (err, results) => {
      ...
      if (ruc && ruc.length !== 11) {
            return res.render("clientes_cajero", {
          clientes: results,
          name: "Administrador",
          alert: true,
...
router.get("/actualizarcliente/:id", requireAuth, async function (req, res) {
  ...
  try {
    ...
    res.render("clientes_cajero", { productos: results });
  } catch (error) {
    console.error(error);
...
router.post("/actualizarcliente/:idcliente", async function (req, res) {
...
router.delete("/eliminarcliente2/:idcliente", async function (req, res) {
  const idCliente = req.params.idcliente;
  try {
    const [results, fields] = await pool.promise().query(
